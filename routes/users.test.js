...
describe("GET /users/:username", function () {
  ...
  test("unauth for not admin & not matching user", async function () {
    const resp = await request(app)
      .get("/users/u1")
      .set("authorization", `Bearer ${u2Token}`);
    expect(resp.statusCode).toEqual(401);
...
describe("POST /users/:username/jobs/:id", () => {
  test("works for admin users", async function () {
    const resp = await request(app)
      .post(`/users/u2/jobs/999999999`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(201);
    expect(resp.body).toEqual({ applied: "999999999" });
  });
  ...
  test("unauth for not admin & not matching", async function () {
    const resp = await request(app)
  ...
  test("not found if user not found", async function () {
    const resp = await request(app)
      .post(`/users/u100/jobs/999999999`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(404);
  });
...
describe("DELETE /users/:username", function () {
  ...
  test("unauth if not admin & not matching user", async function () {
    const resp = await request(app)
      .delete(`/users/u1`)
      .set("authorization", `Bearer ${u2Token}`);
    expect(resp.statusCode).toEqual(401);
